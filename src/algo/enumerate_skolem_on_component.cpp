...
DdNode* Cudd_bddSubstitude(DdManager * dd, DdNode * f, DdNode * x, DdNode * g) {
    ...
    f_0 = Cudd_bddExistAbstract(dd, f_0, x); Cudd_Ref(f_0);
    DdNode* f_1 = Cudd_bddAnd(dd, f, x); Cudd_Ref(f_1);
    f_1 = Cudd_bddExistAbstract(dd, f_1, x); Cudd_Ref(f_1);

    DdNode* f_g = Cudd_bddIte(dd, g, f_1, f_0); Cudd_Ref(f_g);
...
SparseInt Algo::enumerate_skolem_on_component(DdNode* component_y1, DdNode* component_y2) {
    ...
    while (true) {
        ...
        DdNode * accumulated_patches = Cudd_Not(dd_graph_closure.dd->one);
